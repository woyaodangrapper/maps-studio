...
function Loader( editor ) {
	...
	this.loadFile_uri = function ( file ) {
		var filename = file.obj;
		var extension = filename.split( '.' ).pop().toLowerCase();
		...
		switch ( extension ) {
			...
			case 'obj':
				function urlToBlob(file_uri,e) {
					console.log(file_uri)
					let xhr = new XMLHttpRequest();
					xhr.open("get", file_uri, true);
					xhr.responseType = "blob";
					xhr.onload = function () {
						if (this.status == 200) {
							console.log(this.response)
							reader = new FileReader()
							reader.addEventListener( 'load',async function ( event ) {//async 
								var contents = event.target.result;
				...
				urlToBlob(file.mtl,function(mtl){
					urlToBlob(file.obj,function(obj,model){
						var reg = new RegExp("map_Kd ","g");//g,表示全部替换。
						mtl = mtl.replace(reg,"map_Kd "+ file.path + file.img + "/");
						var materials = new model.MTLLoader().parse( mtl );
						var object = new model.OBJLoader().setMaterials( materials ).parse( obj );
						editor.execute( new AddObjectCommand( editor, object ) );
					})
				})
				break;
